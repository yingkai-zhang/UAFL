...
# from ..qrnn3d.layer2 import QRNNConv3D, QRNNDeConv3D, QRNNUpsampleConv3d, BiQRNNDeConv3D, BiQRNNConv3D, BiQRNNUpsampleConv3d
...
#         self.feat_extractor = BiQRNNConv3D(in_channels, channels, bn=bn, act=act)
#         # wo biqrnn
#         # self.feat_extractor = QRNNConv3D(in_channels, channels, bn=bn, act=act)
#         self.layers = nn.ModuleList()
#         # for i in range(num_layer):
#         #     if i not in sample_idx:
#         #         encoder_layer = QRNNConv3D(channels, channels, bn=bn, act=act)
...
#         for i in range(num_layer):
#             if i not in sample_idx:
#                 encoder_layer = BiQRNNConv3D(channels, channels, bn=bn, act=act)
#             else:
#                 encoder_layer = BiQRNNConv3D(channels, 2*channels, k=3, s=(1,2,2), p=1, bn=bn, act=act)
#                 channels *= 2
#             self.layers.append(encoder_layer)
